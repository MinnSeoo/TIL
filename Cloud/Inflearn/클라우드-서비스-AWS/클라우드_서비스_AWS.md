...
RDS - Amazon Relational Database Service (데이터베이스 설치해 주고 대신 운영 해 줌 , 단점 : 비쌈)

아마존은 Managed 라고 소개함

**2) 데이터베이스 생성**

RDS - 여러 개의 주요한 데이터베이스를 빌려주는 서비스

DB 인스턴스 클래스 - 컴퓨터 사양을 지정하는 옵션

다중 AZ 배포 선택시 - 컴퓨터가 서로다른 지역에 총 3개의 Availability Zone(가용성) 을 생성하게 되고 데이터가 들어오게 되면 3개의 지역에 모두 저장이 되어있다가 만약 1군데에서 고장이나도

다른 가용 지역에 있는 데이터베이스 서버에 데이터가 사용된다 이를 통해 데이터의 유실, 서비스 중지를 막는다.  (단점 - 비용이 2배가 든다)

스토리지 - 데이터를 저장하는 디스크

IOPS - 데이터를 저장하고 데이터를 쓸 때 초당 얼마나 작업할 수 있는지 

 (사향이 높을수록 데이터를 처리하는 시간이 더 빠르다. ) 그리고 IOPS는 속도와 용량이 비례하지 않는다 (범용의 경우는 비례함)

**3) 데이터베이스 생성 2**

VPC - 외부로부터 독립된 안전한 네트워크를 구성해 주는 서비스

이 안에 RDS 인스턴스를 만들게 되면 외부에서는 이 VPC를 통해서 직접적으로

RDS에 접속하는 것을 막아줌 (해킹 위험을 낮쳐줌).

또 VPC 안에 aws EC2같은 인스턴스와 같은 컴퓨터를 세팅 후 애플리케이션 서버(Node.js, Python,Php)를 설치하고 이 애플리케이션 서버 만큼은 RDS에 접근할 수 있게 하는것.

**4) Public 방식으로 접속**

인스턴스 → 수정 → 퍼플릭 액세스 가능성 (예) → 계속 → 예약된 다음 유지 관리 기간에 적용(예약된 시간에 변경사항을 수정 후 적용하겠다 (defult)) or 즉시 변경 (상황에 맞게 사용) → DB인스턴스 수정

엔드포인트 - 데이터베이스 서버의 주소 (즉 Host Name)

1) 접속하는 소스를 제약

2) 포트지정

인바운드 - 데이터베이스  서버 입장에서 안으로 들어오는것
